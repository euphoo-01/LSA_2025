...
namespace NasmGen {
    ...
    bool isStandardFunc(const string &name) {
        static const vector<string> stdFuncs = {
            "pow", "sqrt", "getMin", "getMax", "isPrime", "toUpper", "readch", "writech", "writeuint", "writelogic"
        };
        ...
        for (int i = start; i < end; i++) {
            ...
            switch (t.lexema[0]) {
                ...
                case LEX_LESS_OR_EQUAL: {
                    ...
                    string setOp;
                    switch (t.lexema[0]) {
                        case LEX_MORE: setOp = "setg";
                            break;
                        case LEX_LESS: setOp = "setl";
                            break;
                        case LEX_ISEQUAL: setOp = "sete";
                            break;
                        case LEX_NOT_EQUAL: setOp = "setne";
                            break;
                        case LEX_MORE_OR_EQUAL: setOp = "setge";
                            break;
                        case LEX_LESS_OR_EQUAL: setOp = "setle";
                            break;
                    }
                    file << "    " << setOp << " al\n    movzx rax, al\n    push rax\n";
                    stackDepth--;
                ...
                case LEX_DEC: {
                    ...
                    if (k >= start && lex.lexTable.table[k].lexema[0] == LEX_ID && lex.lexTable.table[k].idxTI !=
                        LT_TI_NULLIDX) {
            ...
            switch (t.lexema[0]) {
                ...
                case LEX_ID: {
                    ...
                    if (i + 1 < lex.lexTable.size &&
                        ...
                        int startExpr = i + 2; // сразу после '='
                        int endExpr = startExpr;
                        while (endExpr < lex.lexTable.size &&
                               lex.lexTable.table[endExpr].lexema[0] != LEX_SEMICOLON)
                            endExpr++;
                        ...
                        if (i + 1 < lex.lexTable.size &&
                            (lex.lexTable.table[i + 1].lexema[0] == LEX_INC ||
                             lex.lexTable.table[i + 1].lexema[0] == LEX_DEC)) {
                            IT::Entry &varEntry = lex.idTable.table[t.idxTI];
                            bool isInc = (lex.lexTable.table[i + 1].lexema[0] == LEX_INC);
                            file << "    " << (isInc ? "inc" : "dec")
                                    << " qword [" << getMangledName(varEntry) << "]\n";
                            i += 1; // перепрыгиваем через LEX_INC/LEX_DEC
                        } else {
                            // обычное выражение-оператор
                            int endStmt = i + 1;
                            while (endStmt < lex.lexTable.size &&
                ...
                case LEX_BECAUSE: {
                    ...
                    int j = i + 2;
                    int endInit = j;
                    while (endInit < lex.lexTable.size && lex.lexTable.table[endInit].lexema[0] != LEX_SEMICOLON)
                        endInit++;
                    ...
                    while (endStep < lex.lexTable.size && lex.lexTable.table[endStep].lexema[0] != LEX_RIGHTTHESIS)
                        endStep++;
                    i = endStep;
                    break;
                }
                ...
                case LEX_BRACELET: {
                    if (!blockStack.empty()) {
                        ...
                        if (b.type == 0) {
                            int next = i + 1;
                            while (next < lex.lexTable.size && (
                                       lex.lexTable.table[next].lexema[0] == '\0' || lex.lexTable.table[next].lexema[0]
                                       == LEX_SEMICOLON))
                                next++;
                        ...
                        // конец блока because
                            ...
                            while (stepStart > 0 && lex.lexTable.table[stepStart].lexema[0] != LEX_SEMICOLON)
                                stepStart
                                        --;
                            stepStart++;
                            ...
                            for (int k = stepStart; k < stepEnd; k++)
                                if (lex.lexTable.table[k].lexema[0] == LEX_EQUAL)
                                    isAssign = true;
